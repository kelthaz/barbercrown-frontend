...
export default function AppointmentTable({ appointments }: Props) {
  ...
  useLayoutEffect(() => {
    if (!isMobile && headerCellsRef.current.length > 0) {
      ...
      bodyRows.forEach(row => {
        const cells = row.querySelectorAll('td');
        cells.forEach(cell => {
          cell.style.width = 'auto'; // Resetear anchos en móvil
        });
      });
    }
  ...
  const allColumns = ['Cliente', 'Fecha', 'Hora', 'Estado', 'Funciones'];
            ...
            {appointments
              ...
              .map((appointment) => (
                <TableRow key={appointment.id}>
                  ...
                  <Button size="small" color="primary" aria-label="editar" sx={{ mr: 0 }}> {/* Añadimos un margen derecho */}
                    <EditIcon />
                  </Button>
